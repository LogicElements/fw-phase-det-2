...
/* Internal flags for new config and factory reset */
...
Status_t FlashApp_Init(void)
{
  ...
  /* In case of fault, try it one more time */
  if (ret != STATUS_OK)
  {
    ret = FlashConf_Load();
  }
  ...
  {
    /* No success in reading configuration, erase whole memory */
    FlashConf_Erase();
    FlashConf_Load();
    ret = FlashConf_Store();
    /* Preloaded Factory configuration is used */
  }
...
Status_t FlashApp_Handle(void)
{
  ...
  /* Config requested, set dead time */
  if (flashApp.config)
  {
    flashApp.config = 0;
