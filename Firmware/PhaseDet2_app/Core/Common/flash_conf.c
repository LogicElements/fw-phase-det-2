 ...
 */
#define FLASH_MEM_BLOCK_SIZE          (2*1024)

/**
 * Alignment in memory where new entry can start at
 */
#define FLASH_MEM_PAGE_SIZE           256
...
Status_t FlashConf_Erase(void)
{
  Status_t ret = STATUS_OK;

  ret = System_FlashErase(FLASH_CONF_ADDR_START, FLASH_CONF_ADDR_END - 4);
