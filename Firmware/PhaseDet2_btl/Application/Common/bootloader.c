...
Status_t Btl_Handle(void)
{
  ...
  if (ret == STATUS_OK)
  {
    ...
    ret += System_FlashErase((uint32_t)CONF_C_APPLICATION_OFFSET, (uint32_t)CONF_C_APPLICATION_OFFSET + size - 1);
    ...
    if (Btl_VerifyImage((uint32_t*)CONF_C_APPLICATION_OFFSET) == STATUS_OK)
    {
      ...
      System_FlashErase((uint32_t)CONF_C_APP_BUFFER_OFFSET, (uint32_t)CONF_C_APP_BUFFER_OFFSET + (uint32_t)CONF_C_APP_BUFFER_MAX_SIZE - 1);
    }
  }
  else
  {
